...
class ConvLSTMFastSlow(nn.Module):
    ...
    def forward(self, x, epoch, itr, target, criterion, initial_states=None, rec_steps=None):
        ...
        if initial_states is None:
            ...
            internal_h = initial_states[0]
            internal_c = initial_states[1]
        ...
        if self.grad_method == 'rbp':
            ...
            for i in range(rec_steps):
                states.append([internal_h.detach(), internal_c.detach()])
                (internal_h, internal_c) = self.unit1(x, internal_h, internal_c)
                if rec_steps >= 2:
                    if i == rec_steps - 2:
                        state_2nd_last = internal_h
