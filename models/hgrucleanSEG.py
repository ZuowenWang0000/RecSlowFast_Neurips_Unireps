...
class hConvGRU(nn.Module):
    ...
    def forward(self, x, epoch, itr, target, criterion, initial_state=None):
        x = self.conv0(x)
        x = torch.pow(x, 2)
        ...
        if self.grad_method == 'rbp':
            with torch.no_grad():
                ...
                states.append(internal_state.detach())
        ...
        return output, jv_penalty, loss, states


class hConvGRUFastSlow(nn.Module):

    def __init__(self, filt_size=15, num_iter=50, exp_name='exp1', jacobian_penalty=False, grad_method='bptt'):
        super(hConvGRUFastSlow, self).__init__()
        self.num_iter = num_iter
        self.exp_name = exp_name
        self.jacobian_penalty = jacobian_penalty
        self.grad_method = grad_method
    ...
    def forward(self, x, epoch, itr, target, criterion, initial_state=None, rec_steps=None):
        x = self.conv0(x)
        x = torch.pow(x, 2)
        if initial_state is None:
            internal_state = torch.zeros_like(x, requires_grad=False)
        elif initial_state == 'rand':
            internal_state = torch.randn_like(x, requires_grad=False)
        else:
            internal_state = initial_state
        states = []
        if self.grad_method == 'rbp':
        ...
        jv_penalty = torch.tensor([1]).float().cuda() # dummy value
